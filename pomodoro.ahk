...
ButtonStart:
  ...
  ;startTime := A_Now
  ;endTime := startTime
  ;EnvAdd, endTime, PomodoroTime, m ;adds pomodorotime to current time to get our stop time in YYYYMMDDHH24MISS
  ...
  secs := PomodoroTimeSec
    ...
    /*
      seconds = 1
      timerStarter = false
      onButtonPress(timerStarter = true)
      while (timerStarter == true) {
        wait 1 second
        if (seconds - 1 < 0) {
          if (minutes - 1 < 0) {
            timerStarter = false
...
;TODO - Should be able to count down from a given value on button press and pause- 50:00 - mm:ss format
